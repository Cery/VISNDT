...
/**
 * M8.6 Admin Dashboard E2E Test
 *
 * Covers:
 *   1. No token → 401
 *   2. MEMBER → 403
 *   3. ADMIN → 200 with stats
 */
describe('Admin Dashboard E2E', () => {
  ...
  const TEST_PREFIX = 'E2E_ADM_DASH';
  ...
  describe('Admin Dashboard Stats', () => {
    it('GET /admin/dashboard/stats should return 401 without token', async () => {
      const res = await request(app.getHttpServer()).get('/api/v1/admin/dashboard/stats');
      expect(res.status).toBe(401);
    });

    it('GET /admin/dashboard/stats should return 403 for MEMBER', async () => {
    ...
    it('GET /admin/dashboard/stats should return 200 for ADMIN', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/admin/dashboard/stats')
      ...
      expect(res.body.data).toHaveProperty('demands');
      expect(res.body.data).toHaveProperty('matching');
      expect(res.body.data.users).toHaveProperty('total');
      expect(res.body.data.users).toHaveProperty('active');
      expect(res.body.data.organizations).toHaveProperty('total');
      expect(res.body.data.products).toHaveProperty('total');
      expect(res.body.data.demands).toHaveProperty('total');
      expect(res.body.data.demands).toHaveProperty('published');
      expect(res.body.data.matching).toHaveProperty('totalMatches');
    });
  });
});