...
- **D1 (P2)**: Public catalog queries now `refetchOnWindowFocus: true` (4 surfaces), overriding the global `false` in `providers.tsx`; still bounded by global `staleTime: 60s` so it refetches only when the query has expired on tab-focus return.
...
- Because `staleTime` (60s) and `gcTime` remain global, the refetch is **conservative**: it fires only when the query is already stale (>60s) AND the tab regains focus. No extra network churn for fresh queries.
...
- [providers.tsx](file:///F:/Desktop/VISNDT/VISNDT/apps/web/src/app/providers.tsx#L14-L32)：常驻 Provider 内置路由感知失效器 `CatalogRouteInvalidator`，用 `usePathname` 监听进入公共目录路径 `/`、`/categories`、`/products` 时 `invalidateQueries(['categories']|['featured-products']|['products'])`，命中已常驻观察者强制重拉；跳过首次挂载避免重复抓取。

- [CategorySection.tsx](file:///F:/Desktop/VISNDT/VISNDT/apps/web/src/components/home/CategorySection.tsx#L20-L27)：`refetchOnWindowFocus:true` + `refetchOnMount:'always'`，仍受全局 `staleTime:60s` 约束。
...
D1 source (refetchOnWindowFocus): PASS (4 public catalog surfaces)
D1 source (route-aware invalidator): PASS (providers.tsx CatalogRouteInvalidator)
D1 runtime (SPA 缓存回访):        PASS (真实客户端导航，已删分类不再残留；已补充 invalidate 兜底)
